...
- **gateway**: transactional explode fallback — stage, collision-check, then
  ...
  on remove) so they cannot poison later installs. Update preserves
  `--copy` dest directories instead of rewriting them as store symlinks.
  Vendor installs and first-party installs without a catalog checkout still
  use the skills CLI (no collision doctrine until bake).
