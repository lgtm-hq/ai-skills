...
- `backlog`: Interactive dispatcher that routes a backlog session to implement-issues and/or babysit-pr via a short interview. Applies standing policy (red-main P0, assess before amplifying, CodeRabbit rate-limit override) before spawning lanes. Use when asked to work the backlog, run a backlog session, or dispatch issues and PRs. (`skills/backlog/SKILL.md`)
