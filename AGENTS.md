...
- `lint`: Run linting and formatting with lintro. Use `uv run lintro chk` for checks and `uv run lintro fmt` for formatting. Do not run bundled native tools directly unless another skill explicitly overrides (e.g. raycast). (`skills/lint/SKILL.md`)
