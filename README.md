...
```bash
claude plugin marketplace add lgtm-hq/ai-skills@v0.25.0
...
```bash
bun add -g @lgtm-hq/ai-skills@0.25.0
...
```bash
bunx --package=@lgtm-hq/ai-skills@0.25.0 sk install -y --global \
...
First-party gateway installs resolve `lgtm-hq/ai-skills@v0.25.0`.
...
```bash
gh release download v0.25.0 -R lgtm-hq/ai-skills -p skills-manifest.json
