...
```bash
bun add -g @lgtm-hq/ai-skills@0.16.0
skill          # or: sk
```
...
matches the git tag, `@0.16.0` ↔ `v0.16.0`:

```bash
bunx --package=@lgtm-hq/ai-skills@0.16.0 skill
# or: bunx --package=@lgtm-hq/ai-skills@0.16.0 sk
```
...
```bash
bunx --package=@lgtm-hq/ai-skills@0.16.0 skill install -y --global \
...
```bash
bunx skills add lgtm-hq/ai-skills@v0.16.0 -g
npx skills add lgtm-hq/ai-skills@v0.16.0 -g
pnpm dlx skills add lgtm-hq/ai-skills@v0.16.0 -g
```
...
- **Vendors (v1):** `mattpocock/skills`, `anthropics/skills`,
  ...
  full trees at a **commit SHA** (Renovate bumps SHAs). Skill discovery uses
  baked indexes shipped inside the npm package — no GitHub API at install
  time.
...
```bash
gh release download v0.16.0 -R lgtm-hq/ai-skills -p skills-manifest.json
...
bunx --package=@lgtm-hq/ai-skills@0.16.0 skill install -y --global -a cursor \
  --vendor anthropics --skill frontend-design

# Escape hatch: all first-party skills via skills CLI
bunx skills add lgtm-hq/ai-skills@v0.16.0 -g --all
```

### Update installed skills

Prefer the gateway for installs it manages (globally installed → `skill`; for a
pinned, install-free run swap in `bunx --package=@lgtm-hq/ai-skills@0.16.0 skill`):
